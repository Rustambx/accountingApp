...
const storeHistory = async () => {
    try {
        const res = await axios.post(route('admin.histories.store'), history.value)


        // console.log('История успешно создана:', res.data)

        router.visit(route('admin.histories.index'))
    } catch (e) {
        errors.value = e.response.data.errors

        console.error('Ошибка при создании истории:', e)
    }
}
</script>
...
<template>
    <div>
        <div class="flex items-center justify-between mb-6 border-b pb-3">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Добавление истории</h1>
            </div>
        </div>
        <div>
            <div class="mb-4">
